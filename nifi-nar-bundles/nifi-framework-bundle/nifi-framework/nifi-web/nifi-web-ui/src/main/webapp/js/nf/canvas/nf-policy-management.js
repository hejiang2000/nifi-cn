...
(function (root, factory) {
        ...
        $.widget('nf.userSearchAutocomplete', $.ui.autocomplete, {
            ...
            _renderMenu: function (ul, items) {
                ...
                if (ul.children().length === 0) {
                    ul.append('<li class="unset search-no-matches">无匹配搜索条件的用户</li>');
                }
            },
    ...
    var initPolicyTable = function () {
                        ...
                        if (option.value === 'restricted-components') {
                            if (!initializedComponentRestrictions) {
                                ...
                                var options = [{
                                    ...
                                    value: '',
                                    description: '允许用户创建/修改所有限制组件, 不管任何限制规则.'
                                }];
                                ...
                                requiredPermissions.each(function (label, id) {
                                    if (id !== option.value) {
                                        options.push({
                                            text: "需要 '" + label + "'",
                                            value: id,
                                            description: "允许用户创建/修改限制组件, 需要 '" + nfCommon.escapeHtml(label) + "'"
                                        });
                                    }
                                });
        ...
        $('#component-policy-target').combo({
            options: [{
                ...
                description: '允许用户查看组件配置详细信息'
            }, {
                text: '修改组件',
                value: 'write-component',
                description: '允许用户修改组件配置详细信息'
            }, {
                text: '操作组件',
                value: 'operate-component',
                description: '允许用户通过修改组件运行状态(启动/停止/启用/禁用), 远程端口传输状态, 终止处理器线程来操作组件'
            }, {
                text: '查看朔源',
                value: 'read-provenance',
                description: '允许用户查看该组件产生的溯源事件'
            }, {
                text: '查看数据',
                value: 'read-data',
                description: '允许用户通过输出连接 FlowFile 队列或溯源事件查看该组件的元数据和正文'
            }, {
                text: '修改数据',
                value: 'write-data',
                description: '允许用户清空输出连接的 FlowFile 队列, 并通过溯源事件提交重放'
            }, {
                text: '经由 site-to-site 接收数据',
                value: 'write-receive-data',
                description: '允许该端口从这些 NiFi 实例接收数据',
                disabled: true
            }, {
                text: '经由 site-to-site 发送数据',
                value: 'write-send-data',
                description: '允许该端口发送数据到这些 NiFi 实例',
                disabled: true
            }, {
                text: '查看策略',
                value: 'read-policies',
                description: '允许用户查看能够查看/修改该组件的用户列表'
            }, {
                text: '修改策略',
                value: 'write-policies',
                description: '允许用户修改查看/修改组件的用户列表'
            }],
            select: function (option) {
                if (initialized) {
    ...
    var promptToRemoveUserFromPolicy = function (item) {
        nfDialog.showYesNoDialog({
            headerText: '更新策略',
            dialogContent: '从该策略中移除 \'' + nfCommon.escapeHtml(item.component.identity) + '\'?',
            yesHandler: function () {
    ...
    var promptToDeletePolicy = function () {
        nfDialog.showYesNoDialog({
            headerText: '删除策略',
            dialogContent: '通过删除该策略, 该组件权限将会恢复到可用的继承策略.',
            yesText: '删除',
            noText: '取消',
            yesHandler: function () {
                deletePolicy();
    ...
    var getResourceMessage = function (resource) {
        if (resource === '/policies') {
            return $('<span>显示从所有策略继承来的有效策略.</span>');
        } else if (resource === '/controller') {
            return $('<span>显示从控制器继承的有效策略.</span>');
        } else if (resource === '/parameter-contexts') {
            return $('<span>显示从全局参数上下文继承的有效策略.</span>');
        } else {
            ...
            // build the mark up
            return $('<span>显示从处理组继承的有效策略 </span>')
                ...
                $.ajax({
                    ...
                    if (policyEntity.permissions.canRead === true) {
                        ...
                        if (resourceAndAction.resource === policy.resource) {
                            ...
                            $('#policy-message').text('未指定组件特定管理员.');
                    ...
                    if (xhr.status === 404) {
                        ...
                        // show an appropriate message
                        $('#policy-message').text('没有访问特定资源策略的权限.');
                ...
                $.ajax({
                    ...
                    if (xhr.status === 404) {
                        ...
                        if (resourceAndAction.resource === '/restricted-components') {
                            $('#policy-message').text('无"不受限制"权限用户');
                        } else {
                            $('#policy-message').text('无特定权限用户.');
                        }
